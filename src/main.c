...
int main(int argc, char *argv[]) {
  ...
  unsigned char smart_read_cdb[CMD_LENGTH] = {0xa1, 0x0c, 0x0e, 0xd0, 1, 0,
                                              0x4f, 0xc2, 0,    0xb0, 0, 0};
  ...
  // extracting SMART attributes double raw;
  int id;
  ...
  for (register int i = 2; i < 361; i = i + 12) {
    ...
    long long int raw = 0;
    raw = (long long int)((buffer[i + 5]) | (buffer[i + 6] << 8) |
                          (buffer[i + 7] << 16) | (buffer[i + 8] << 24) |
                          ((long long int)buffer[i + 9] << 32) |
                          ((long long int)buffer[i + 10] << 40));
